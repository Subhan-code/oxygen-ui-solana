...
export function BlurShimmerText({
  ...
  const Component = as as typeof motion.span;
  ...
  return (
    <span className={cn("inline-grid text-center max-w-full align-top", className)}>
      {texts.map((text, index) => (
        <span
          key={index}
          className="invisible col-start-1 row-start-1 block whitespace-normal sm:whitespace-nowrap select-none"
          aria-hidden="true"
        >
      ...
      <AnimatePresence mode="wait">
        <Component
          key={currentIndex}
          className="col-start-1 row-start-1 block whitespace-normal sm:whitespace-nowrap"
          initial="initial"
          animate="animate"
          exit="exit"
        >
