...
export function Field({
  ...
  const formattedUsd = formatAmount(usdValue, 2);
  const formattedBalance = token.balance ? formatAmount(token.balance) : "0.00";
        ...
        <div className="min-w-0 flex-1">
          {editable ? (
            ...
            <div className="flex h-9 items-center gap-2 text-2xl font-semibold tracking-tight tabular-nums">
              ...
              >
                {amount ? <DigitSwap value={amount} /> : "0"}
              </motion.span>
              <AnimatePresence>
                {quoting ? (
                  <motion.span
          ...
          <p className="mt-1 flex items-center text-[11px] text-muted-foreground tabular-nums">
            <span>≈ $</span>
            <DigitSwap value={formattedUsd} />
          </p>
        </div>
      ...
      <div className="mt-2 flex items-center justify-between text-[11px] text-muted-foreground">
        <span className="inline-flex items-center gap-1">
          ...
          <span className="tabular-nums">
            <DigitSwap value={formattedBalance} />
          </span>
