...
export function CryptoSpeedDialActionMenu({ className, ...props }: CryptoSpeedDialActionMenuProps) {
  ...
  return (
    <div
      data-slot="root"
      className={cn(
        "relative mx-auto flex w-full max-w-sm flex-col items-center justify-center p-8 rounded-3xl bg-zinc-950 border border-zinc-800 font-sans min-h-[200px]",
        className
      )}
      {...props}
    >
      <div className="relative">
        <button
          ...
          className={cn(
            "flex h-11 w-11 items-center justify-center rounded-full bg-white text-zinc-950 shadow-md transition-transform cursor-pointer z-20 hover:bg-zinc-200",
            open && "rotate-45"
          )}
        >
