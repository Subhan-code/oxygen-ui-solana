...
export function CryptoWalletDashboard({
  ...
  return (
    <div
      data-slot="root"
      className={cn(
        "relative mx-auto flex w-full max-w-sm flex-col overflow-hidden rounded-[36px] bg-black text-white shadow-2xl border border-zinc-800/80 font-sans min-h-[680px]",
        className
      )}
      {...props}
    >
      <div className="flex items-center justify-between gap-1 p-3 pb-2 pt-4 px-4">
        <button
          ...
          className="flex h-10 w-10 items-center justify-center rounded-full bg-gradient-to-tr from-purple-500 via-teal-400 to-emerald-300 transition-transform active:scale-95 cursor-pointer shadow-md"
          aria-label="Open Solana wallet menu"
        >
        ...
        <nav className="flex items-center gap-1 overflow-x-auto rounded-full bg-zinc-900/90 p-1 no-scrollbar border border-zinc-800/60">
          {(["Home", "Trade", "Predict", "Explore"] as const).map((tab) => (
            <button
              ...
              className={cn(
                "relative rounded-full px-3.5 py-1.5 text-xs font-semibold transition-colors cursor-pointer",
        ...
        <button
          ...
          className="flex h-8 w-8 items-center justify-center rounded-full bg-zinc-800 text-zinc-300 hover:bg-zinc-700 transition-colors text-xs cursor-pointer"
      ...
      <div className="flex flex-1 flex-col p-4 pt-2">
        <div className="relative mb-4">
          ...
          {accountMenuOpen && (
            <div className="absolute left-0 top-7 z-30 w-48 rounded-xl border border-zinc-800 bg-zinc-900 p-1.5 shadow-xl backdrop-blur-md">
              {accounts.map((acc) => (
                <button
                  key={acc}
                  type="button"
        ...
        <div className="mb-4 flex items-center justify-between rounded-2xl border border-zinc-800/80 bg-zinc-900/60 p-3.5 transition-all hover:border-zinc-700">
        ...
        <div className="space-y-2 mb-4">
          <div className="flex items-center justify-between rounded-2xl border border-zinc-800/60 bg-zinc-900/40 p-3 transition-colors hover:bg-zinc-900/80">
            <div className="flex items-center gap-3">
          ...
          <div className="flex items-center justify-between rounded-2xl border border-zinc-800/60 bg-zinc-900/40 p-3 transition-colors hover:bg-zinc-900/80">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-zinc-400">{usdcBalance.toLocaleString()} USDC</p>
              </div>
            </div>
            <div className="text-right">
        ...
        <div className="grid grid-cols-2 gap-2.5 mb-auto">
          <div className="h-24 rounded-2xl border border-zinc-800/60 bg-zinc-900/40 p-3 flex flex-col justify-between">
            <span className="text-xs font-medium text-zinc-400">SOL Long 10x</span>
            <span className="text-sm font-bold text-emerald-400">+24.8%</span>
          </div>
          <div className="h-24 rounded-2xl border border-zinc-800/60 bg-zinc-900/40 p-3 flex flex-col justify-between">
            <span className="text-xs font-medium text-zinc-400">JUP Short 5x</span>
            <span className="text-sm font-bold text-rose-400">-2.1%</span>
          </div>
        </div>
      </div>
      ...
      <div className="p-3 bg-black/90 backdrop-blur-md flex items-center gap-2 border-t border-zinc-900">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full rounded-full border border-zinc-800/80 bg-zinc-900/90 py-2 pl-9 pr-3 text-xs text-zinc-200 placeholder-zinc-500 focus:outline-none focus:ring-1 focus:ring-emerald-400"
          />
        </div>
        <button
          type="button"
          onClick={() => setSettingsOpen(true)}
          className="flex h-9 w-9 items-center justify-center rounded-full bg-emerald-400 text-zinc-950 hover:bg-emerald-300 transition-colors shadow-lg active:scale-95 cursor-pointer"
      ...
      <AnimatePresence>
        {drawerOpen && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="absolute inset-0 z-40 bg-black/80 backdrop-blur-sm flex"
          >
            <motion.div
              initial={{ x: "-100%" }}
              animate={{ x: 0 }}
              ...
              className="w-4/5 h-full bg-black p-5 border-r border-zinc-800 flex flex-col justify-between"
            >
              <div>
                <div className="flex items-center justify-between mb-6">
                  <div className="flex items-center gap-2">
                    <div className="h-10 w-10 rounded-full bg-gradient-to-tr from-purple-500 via-teal-400 to-emerald-300 flex items-center justify-center text-sm font-bold text-black">
                      ⚡
                    </div>
                    <div>
                      <h3 className="text-base font-bold text-white">alex.sol</h3>
                      <p className="text-xs text-emerald-400">Solana Mainnet-Beta</p>
                    </div>
                  </div>
                  <button
                    type="button"
                ...
                <div className="space-y-1">
                  {[
                    ...
                    <button
                      ...
                      className="flex w-full items-center gap-3.5 rounded-xl px-3 py-2.5 text-sm font-semibold text-zinc-200 hover:bg-zinc-900 transition-colors cursor-pointer"
                    >
      ...
      <AnimatePresence>
        {settingsOpen && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="absolute inset-0 z-50 bg-black flex flex-col p-4 overflow-y-auto"
          >
            ...
            <div className="mb-4 flex items-center justify-between rounded-2xl bg-zinc-900 p-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <span className="text-sm font-bold text-white block">alex.sol</span>
            ...
            <div className="space-y-4">
              ...
              <div className="rounded-2xl bg-zinc-900 p-3 flex items-center justify-between">
                <div className="flex items-center gap-3 text-sm text-zinc-200">
                  <Code className="h-4 w-4 text-zinc-400" />
      ...
      <AnimatePresence>
        {lightMenuOpen && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="absolute inset-0 z-50 bg-zinc-100 p-5 flex flex-col justify-between text-zinc-900"
          >
            <div>
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-2">
                ...
                <div className="flex items-center gap-2">
                  <button
                    type="button"
                    className="rounded-full bg-emerald-200 px-4 py-1.5 text-xs font-bold text-emerald-950 cursor-pointer"
                  >
                    Connect Wallet
                  </button>
                  <button
