...
export function CryptoWalletMain({
  ...
  return (
    <div
      data-slot="root"
      className={cn(
        "relative mx-auto flex w-full max-w-sm flex-col overflow-hidden rounded-3xl bg-zinc-950 text-white shadow-xl border border-zinc-800 font-sans min-h-[620px]",
        className
      )}
      {...props}
    >
      <div className="flex items-center justify-between gap-2 p-4 pb-2 border-b border-zinc-900">
        <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-zinc-900 border border-zinc-800 text-white font-bold text-sm">
          ⚡
        </div>
          ...
          {(["Home", "Trade", "Predict", "Explore"] as const).map((tab) => (
            <button
              ...
              className={cn(
                ...
                activeTab === tab ? "bg-white text-zinc-950" : "text-zinc-400 hover:text-zinc-200"
              )}
            >
              {tab}
            </button>
        ...
        <div className="flex h-8 w-8 items-center justify-center rounded-full bg-zinc-900 border border-zinc-800 text-zinc-400 text-xs cursor-pointer">
          🌐
        </div>
      </div>
      ...
      <div className="flex flex-1 flex-col p-4 pt-3">
        <div className="relative mb-4">
              ...
              {accounts.map((acc) => (
                <button
                  ...
                  className={cn(
                    ...
                    selectedAccount === acc ? "bg-zinc-800 text-white" : "text-zinc-400 hover:bg-zinc-800/60"
                  )}
                >
                  {acc}
                </button>
          ...
          <div className="mt-1 flex items-center gap-2 text-xs font-medium">
            <span className="text-emerald-500">+${changeUsd.toFixed(2)}</span>
            <span className="rounded-md bg-emerald-950/60 border border-emerald-800/40 px-1.5 py-0.5 text-emerald-400 font-semibold">
              +{changePercent.toFixed(2)}%
            </span>
          </div>
        </div>

        <div className="mb-4 flex items-center justify-between rounded-2xl border border-zinc-800 bg-zinc-900/80 p-3.5">
        ...
        <div className="space-y-2 mb-4">
          <div className="flex items-center justify-between rounded-2xl border border-zinc-800 bg-zinc-900/60 p-3">
            ...
            <div className="text-right">
              ...
              <span className="text-[11px] font-semibold text-emerald-500">+2.4%</span>
            </div>
          </div>

          <div className="flex items-center justify-between rounded-2xl border border-zinc-800 bg-zinc-900/60 p-3">
            <div className="flex items-center gap-3">
              <div className="flex h-9 w-9 items-center justify-center rounded-full bg-blue-900/40 border border-blue-700/50 text-blue-400 font-bold text-xs">
                USDC
              </div>
              <div>
            ...
            <div className="text-right">
              ...
              <span className="text-[11px] font-semibold text-emerald-500">+0.01%</span>
            </div>
          </div>
        </div>
      </div>
