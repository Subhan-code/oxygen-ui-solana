...
const BAR = "bg-neutral-900/80 dark:bg-black/60 border border-white/10 backdrop-blur-xl";
const BAR_TEXT = "text-neutral-900 dark:text-black";
...
const SIZES = {
  ...
  md: {
    label: "gap-2 px-4 py-2 text-xs leading-5 [&_svg]:size-3.5",
    radius: 9999,
    separation: 14,
  },
  lg: {
...
function NavLabel({
  ...
  const props = {
    ...
    className: cn(
      "flex cursor-pointer items-center whitespace-nowrap font-semibold [&_svg]:shrink-0 rounded-full transition-colors",
...
export function GooeyNav({
  ...
  const fill = (i: number) => (i === active ? activeColor : "transparent");
        ...
        {items.map((item, i) => {
          ...
          return (
            <Segment
              ...
              className={cn(BAR, "rounded-full transition-all duration-200", isActive ? FADE_IN : FADE_OUT)}
