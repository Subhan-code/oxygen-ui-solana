...
const TONES = {
  none: { bar: "bg-zinc-300 dark:bg-white/20", text: "text-zinc-500 dark:text-zinc-400" },
    ...
    <div className={cn("w-full font-sans select-none", className)}>
      ...
      >
        {Array.from({ length: max }, (_, i) => (
          <div
            key={i}
            className="relative h-1.5 overflow-hidden rounded-[2px] bg-zinc-200 dark:bg-white/10"
          >
            <motion.span
              className={cn("absolute inset-0 origin-left rounded-[2px] transition-colors duration-200", tone.bar)}
              initial={false}
      ...
      <div className="mt-2 flex h-5 items-center justify-between gap-3">
        <span className="inline-grid text-xs font-semibold leading-5">
          {labels.map((text, i) => (
            <motion.span
              key={text}
              aria-hidden
              className={cn("col-start-1 row-start-1 whitespace-nowrap transition-colors duration-200", tone.text)}
              initial={false}
        ...
        <motion.span
          aria-hidden
          className="whitespace-nowrap text-[11.5px] leading-5 text-amber-600 dark:text-amber-400 font-medium"
          initial={false}
        ...
        <ul className="mt-3 grid gap-1.5">
          {evaluated.map((rule) => (
            <li key={rule.id} className="flex items-center gap-2">
              <span className="relative grid size-[14px] shrink-0 place-items-center rounded-[4px] border border-zinc-300 text-white dark:border-zinc-700 dark:text-zinc-950">
                <motion.span
              ...
              <span
                className={cn(
                  "text-xs leading-5 transition-colors duration-200 font-medium",
                  rule.met
