...
import { CheckCircle2, Clock, AlertCircle, RefreshCw, Calendar, Repeat, Video, Navigation, User } from "lucide-react";
...
export function SignatureStatusBadge({
  ...
  const configs: Record<CommitmentStatus, { label: string; icon: any; style: string }> = {
    finalized: { label: "Finalized", icon: CheckCircle2, style: "bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border-emerald-500/30" },
    confirmed: { label: "Confirmed", icon: CheckCircle2, style: "bg-blue-500/10 text-blue-600 dark:text-blue-400 border-blue-500/30" },
    pending: { label: "Pending", icon: RefreshCw, style: "bg-amber-500/10 text-amber-600 dark:text-amber-400 border-amber-500/30" },
    failed: { label: "Failed", icon: AlertCircle, style: "bg-rose-500/10 text-rose-600 dark:text-rose-400 border-rose-500/30" },
  };
  ...
  return (
    <div
      className={cn(
        "inline-flex items-center gap-2 rounded-full border px-3 py-1 text-xs font-mono font-semibold shadow-xs backdrop-blur-md font-sans select-none",
        config.style,
        className
      )}
    ...
    >
      ...
      {status === "confirmed" && (
        <span className="text-[10px] opacity-70">({confirmations}/{maxConfirmations})</span>
      )}
    </div>
  );
}
...
export function EventTagPills({
  ...
  return (
    <div
      ...
      className={cn("flex flex-wrap items-center justify-center gap-3 p-2 select-none font-sans", className)}
      {...props}
    >
      {tags.map((tag) => {
        ...
        return (
          <motion.button
            ...
            className={cn(
              "inline-flex items-center gap-2 rounded-full px-4 py-2 text-xs font-semibold transition-all duration-200 cursor-pointer shadow-2xs border border-transparent",
