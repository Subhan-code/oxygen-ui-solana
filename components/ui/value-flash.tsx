...
export function ValueFlash({
  ...
  return (
    <motion.span
      ...
      className={cn(
        "relative inline-grid grid-flow-col items-center gap-1.5 rounded-md px-2 py-1 text-xs font-bold tabular-nums font-mono transition-colors duration-200 select-none",
        tone,
        className
      )}
    >
      {direction ? (
        <motion.span
          ...
          className={cn("pointer-events-none absolute inset-0 rounded-md", tint)}
        />
      ) : null}
